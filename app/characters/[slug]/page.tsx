...
export default async function CharacterPage({ params }: CharacterPageProps) {
  ...
  after(() => {
    void recordPageView({
      ...
      console.error("Failed to record character page view", error);
    });
  });
          ...
          <div className="mt-8 flex max-w-3xl flex-col items-start gap-6">
            ...
            <div className="flex w-full flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center">
              <div className="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-shah-gold-400/20 bg-black/30 px-5 py-3 text-sm font-black text-shah-gold-100 shadow-[0_12px_35px_rgba(0,0,0,0.25)] backdrop-blur-xl sm:w-auto">
                <span>{character.dynasty || "شخصیت شاهنامه"}</span>
              </div>
              ...
              >
                ...
                <span className="relative inline-flex w-full items-center justify-center gap-3 overflow-hidden rounded-2xl border border-white/10 bg-shah-black-950/85 px-7 py-3.5 text-sm font-black text-shah-gold-300 shadow-[0_18px_50px_rgba(0,0,0,0.35)] backdrop-blur-xl transition-all duration-500 group-hover:border-shah-gold-400/40 group-hover:bg-shah-gold-500 group-hover:text-shah-black-950 group-hover:shadow-[0_22px_70px_rgba(234,179,8,0.22)] sm:w-auto sm:px-8 sm:py-4 sm:text-base">
                  ...
                  <span className="relative">آشنایی با {character.name}</span>
...
function QuickInfo({
  ...
  return (
    <section className="relative overflow-hidden rounded-[2.35rem] border border-shah-gold-500/25 bg-linear-to-br from-white via-shah-cream-50 to-shah-gold-50/70 p-3 text-right shadow-2xl shadow-shah-black-900/10 backdrop-blur-md md:backdrop-blur-xl dark:border-shah-gold-400/25 dark:from-white/[0.07] dark:via-[#101010]/95 dark:to-black/70 dark:shadow-black/50 sm:p-4">
...
function TextCard({
  ...
  return (
    <section className="relative overflow-hidden rounded-[1.7rem] border border-shah-gold-500/20 bg-white/85 p-5 text-right shadow-xl shadow-shah-black-900/8 backdrop-blur-md md:backdrop-blur-xl dark:bg-[#101010]/90 dark:shadow-black/40 md:p-6">
...
function SideCard({ title, children }: { title: string; children: ReactNode }) {
  return (
    <section className="rounded-[1.7rem] border border-shah-gold-500/20 bg-white/85 p-5 text-right shadow-xl shadow-shah-black-900/8 backdrop-blur-md md:backdrop-blur-xl dark:bg-[#101010]/90 dark:shadow-black/40 md:p-6">
      <SectionTitle>{title}</SectionTitle>
      {children}
    </section>
  );
